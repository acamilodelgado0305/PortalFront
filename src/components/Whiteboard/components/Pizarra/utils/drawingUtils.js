...
export const handleMouseDown = (e, setIsDrawing, setCurrentLine) => {
  ...
  export const handleMouseUp = (isDrawing, drawingMode, currentLine, lines, setLines, setCurrentLine, setIsDrawing) => {
    if (!isDrawing) return;
    setIsDrawing(false);
    if (drawingMode === 'draw') {
  ...
  export const handleMouseMove = (isDrawing, drawingMode, currentLine, lines, setCurrentLine, setLines, e) => {
    if (!isDrawing) return;
    const pos = e.target.getStage().getPointerPosition();
    if (drawingMode === 'draw') {
