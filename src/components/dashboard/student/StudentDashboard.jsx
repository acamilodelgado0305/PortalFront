...
import { getUpcomingClasses, getNextClass, convertToLocalTime, getActiveClasses, sortClassesByDate } from "../../../helpers";
...
<div className="flex">
  ...
  {/* Upcoming Activities */}
<div className="rounded-lg bg-white p-6 shadow-lg">
  <h2 className="mb-4 text-xl font-semibold text-blue-600">Todas tus clases</h2>
  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
    {/* Asegurándonos de que allClasses sea un array válido antes de hacer .slice() */}
    {(showAllOldClasses ? allClasses : (allClasses || []).slice(0, 3)).map((professor) => (
      <Card key={professor.teacher.id} className="shadow-md transition-all hover:shadow-lg">
        <div className="flex flex-col p-4">
          <span className="text-lg font-semibold">{professor.teacher.firstName}</span>
          <span className="text-gray-600">{professor.teacher.subjectYouTeach}</span>
  ...
  {allClasses && allClasses.length > 3 && (
    <button onClick={() => setShowAllOldClasses(!showAllOldClasses)} className="mt-4 text-blue-500 hover:underline">
